...
function findDividableBy(array:number[], dividableBy:number) {
    var lowestNumber = null, highestNumber = null;
    var dividableByNumber:number[] = [];

    for (let i = 0; i < array.length; i++) {
        let n = array[i];
        if(lowestNumber == null) lowestNumber = n;
        if(highestNumber == null) highestNumber = n;
        
        if(n < lowestNumber) lowestNumber = n;
        if(n > highestNumber) highestNumber = n;

        if(n%dividableBy == 0) dividableByNumber.push(n);
    }
...
function getRandom(range:number):number {
